...
async function getAPI() {
	...
	for (var i = 0; i < item.length; i++) {
		var apiItem = `<div class="col-md-4 bg-dark d-block m-auto rounded api-task p-3">
					<img src="${item[i].url}" class="img-fluid m-auto" />
					<h3 class="text-center text-light m-1">${item[i].id}</h3>
					<h5 class="text-center text-light m-1">${item[i].title}</h5>
				</div>`;
		result += apiItem;
	}

	document.querySelector(".api-task").innerHTML = result;
}
getAPI();
